...
{
    int tailleExpression = strlen(expression) - 1;
...
Bool verifExpressionCalcul(char* calcul)
{
    ...
    for(indice = 0; indice < strlen(calcul); indice++)
    {
...
Bool verifPasAlpha(char* expression)
{
    ...
    for(indice = 0; indice < strlen(expression); indice++)
    {
...
int postVerification(char* calcul)
{
    ...
    Bool erreurOperateur;
    Bool erreurLettre;
    Bool erreurFinCalcul;
